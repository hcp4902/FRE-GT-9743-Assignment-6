...
class ValuationEngineProductOvernightIndexBasisSwap(ValuationEngineProduct):
    ...
    def calculate_value(self):
        ...
        self.float_1_value_ = self.floating_leg_1_wo_basis_engine_.value_ + self.floating_leg_1_basis_engine_.value_
        self.float_2_value_ = self.floating_leg_2_engine_.value_
        self.float_1_cash_ = self.floating_leg_1_wo_basis_engine_.cash_ + self.floating_leg_1_basis_engine_.cash_
        self.float_2_cash_ = self.floating_leg_2_engine_.cash_
        self.annuity_ = self.floating_leg_1_basis_engine_.value_ / self.spread_

        self.value_ = (
            self.floating_leg_1_sign_ * self.float_1_value_ + self.floating_leg_2_sign_ * self.float_2_value_
        )
        self.cash_ = (
            self.floating_leg_1_sign_ * self.float_1_cash_ + self.floating_leg_2_sign_ * self.float_2_cash_
        )
    ...
    def create_cash_flows_report(self) -> CashflowsReport:
        ...
        # floating leg-1
        n_fix = self.product_.floating_leg_1.num_cashflows()
        for i in range(n_fix):
            ...
            this_cf.add_row(
                ...
                (
                    (self.floating_leg_1_basis_engine_.payoffs_[i] + self.floating_leg_1_wo_basis_engine_.payoffs_[i]) * self.floating_leg_1_wo_basis_engine_.dfs_[i]
                    if self.value_date < pay_date
                    else (
                        (self.floating_leg_1_basis_engine_.payoffs_[i] + self.floating_leg_1_wo_basis_engine_.payoffs_[i])
                        if self.value_date == pay_date
                ...
                self.floating_leg_1_basis_engine_.dfs_[i],
                fixing_date=getattr(cf, "termination_date", None),
                start_date=getattr(cf, "effective_date", None),
                end_date=getattr(cf, "termination_date", None),
                accrued=self.floating_leg_1_wo_basis_engine_.accruals_[i],
                index_or_fixed=self.spread_,
                index_value=self.floating_leg_1_wo_basis_engine_.payoffs_[i],
            )

        # floating leg-2
        n_fix = self.product_.floating_leg_2.num_cashflows()
        for i in range(n_fix):
    ...
    def grad_at_par(self) -> np.ndarray:
        ...
        """
        db = d(V2)/annuity - d(V1_wo_basis)/annuity + (V2 - V1_wo_basis) *(1/annuity^2)d(annuity)
        d(annuity) =  d(V1_basis / spread_over_leg_1) =  d(V1_basis) / spread_over_leg_1
        """

        a =  self.floating_leg_1_basis_engine_.value_ / self.spread_
        self.floating_leg_2_engine_.calculate_first_order_risk(
            local_grad, scaler=1.0 / a, accumulate=True
        )
        self.floating_leg_1_wo_basis_engine_.calculate_first_order_risk(
        ...
        self.floating_leg_1_basis_engine_.calculate_first_order_risk(
            local_grad, scaler=- (self.floating_leg_2_engine_.value_ - self.floating_leg_1_wo_basis_engine_.value_ ) / self.spread_ / a / a, accumulate=True
        )
